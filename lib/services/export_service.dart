...
class ExportService {
  ...
  Future<void> exportMultipleReceiptsToPDF(List<Receipt> receipts) async {
    try {
      final pdf = pw.Document();
      ...
      // Save PDF
      final output = await getTemporaryDirectory();
      final file = File('${output.path}/bulk_receipts_${DateTime.now().millisecondsSinceEpoch}.pdf');
      await file.writeAsBytes(await pdf.save());
      
      // Share PDF
      await Share.shareXFiles([XFile(file.path)], text: 'Bulk Receipts Export');
      
    } catch (e) {
      throw Exception('Failed to generate bulk PDF: $e');
    }
  }
